...
- **`--dry-run` が `--unpack` / `--repack` モードで無視される問題を修正** — これまで両モードは `--dry-run` を無視し、実際に解凍・パッキングしてディスクへ書き込んでいた。入口で `dry_run` を優先判定し、解凍/パッキング計画のみを表示して書き込みを行わない（`--json` / `--json-out` 出力を含む）。help 契約「解凍・パッキング・出力ディレクトリ作成は行いません」に準拠
  
  #### メンテナンス
  
- **解凍先ディレクトリ名の決定を `_unpack_target_dir` に抽出** — 実際の解凍と `--dry-run` プレビューが同一の命名規則を共有し、試運転プレビューが実実行と一致

### [3.5.3] - 2026-09-02

#### 修正

- **`--list-images` の不正なフィルタ式での終了コードを修正** — 不正な式の分岐が裸の `return` だったのを `return 1` に変更。`_main` が空リストに対して `max(mode_codes)` を呼んで TypeError になるのを回避
- **`get_drm_flag` の EXTH オフセットを修正** — DRM フラグを読み取る EXTH アイテム内オフセットを `+14` から `+12` に修正（`0x0E` → `0x0C`）

### [3.5.2] - 2026-09-01

#### メンテナンス

- **`_list_ebook` が `ensure_cover_first` を再利用** — 重複した表紙フォールバックスキャンを削除（ファイル名キーワードによる表紙補完を単一実装に統合）
- **`parse_inspect_arg` のローカル変数 `t` を `tok` に改名** — i18n 翻訳関数 `t()` のシャドーイングを解消
- **`emit` のログを永続ハンドルに変更** — 書き込みごとの open/close を解消
...
- **`--repack` の処理リストに推測出力名を明記** — 実行前に出力する処理リストの各行に `解凍ディレクトリ → 出力.cbz` を表示し、推測元が一目瞭然（`--repack` モード内部のリスト表示であり、独立したパラメータではありません）
