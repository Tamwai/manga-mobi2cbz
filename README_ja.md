...
- **画像一覧の読み取り専用表示** — `--list-images [FILTER]` で対象電子書籍の全画像を（No. / ファイル名 / 解像度 / サイズ / モード・深度 / 向き / ディレクトリ / タグ）で列挙し、全量統計ブロック（形式 / モード・深度 / 寸法分布 / 見開き横幅 / 動図 GIF / 小画像 / 異常明細）を付加。変換せず、CBZ 書き込みも ComicInfo 生成もしません。FILTER は任意で条件式をサポート（形式 / `res` / `size` / 向き / モード / ビット深度 / タグ / `small[=比率]`。カンマ = OR、`+` = AND、`-` 接頭辞 = 除外）。EXIF Orientation≠1 の画像には `[回転N]` マーク（N=2~8、正常=1 は無印）を付け `[異常]` 集計にも加算。方向ワードでフィルタ: 値なし = 方向異常の全画像を選択（rotate=auto で回転予定）、値あり = EXIF Orientation の正確一致（例: `方向=6`）。`--json-out` は `orientation` フィールド（v3.6.0）
- **ファイル名 glob フィルタ** — `name=` の値に `*` / `?` を含む場合は glob 一致でファイル名をマッチ（大文字小文字を無視、特別扱いは `*` と `?` のみ、`[]` はリテラル）、含まない場合は従来の部分一致を維持。`--list-images` / `--drop` / `--pages` の三入口で有効、例 `name=*_表紙*`、`name=p00?`
- **画像処理パイプライン** — `--img-edit` で出力画像に回転 / 鏡像反転 / グレースケール化 / 形式変換 / 再エンコード品質制御 / 拡縮 / 白辺トリミング / EXIF 除去を適用（繰り返し指定可、値内の '+' で分割してフラット化）：`rotate[=auto|90|180|270]` は EXIF Orientation をピクセルに焼き込み Orientation フィールドを削除して再エンコード（値なし = auto、Orientation≠1 の画像のみ処理）、`flip=x|y|both` は鏡像反転（x = 水平、y = 垂直、both = 両方）、`grayscale` はグレースケール化（常に flip の直後に挿入、出力は RGB 三チャンネル等値）、`format=jpeg|png|webp`（別名 jpg；目標形式、再エンコード後に cbz 内エントリの拡張子を同期変更；透明→JPEG はデフォルトで白を補填、`format=jpeg,black` や `#RRGGBB` で色変更可；JPEG/WebP は quality で保存、PNG は可逆）、`quality=1-100` は再エンコード品質を上書き（デフォルト 95、範囲外はエラー）、`scale=NNN%` はパーセント相対拡縮（ページ間の比率を維持、例 `200%`=2 倍、1~1000）または `scale=NNNw` は目標幅ピクセルに統一（高さは元の比率、例 `1200w`、1~100000、二つの構文は二択）、`trim[=許容差]` は辺縁の均一背景色を検出して白辺を自動トリミング（値なし/auto = 0.05、安全マージンで誤トリミング防止）、`strip` は画像の全 EXIF メタデータを除去。固定順序 `rotate→flip→grayscale→format→quality→scale→trim→strip` は記述順に関係なし。三つの入口：mobi→cbz 変換出力前 / `--repack` パッキング前 / 既存 `.cbz` のその場修正（CBZ 方向修正モード）。`--dry-run`/`--json`/`--json-out`/`--log` と連動
...
- Python 3.10+
- 実行時依存: `mobi`（必須）
- オプション: `pillow`（`--img-edit` 画像パイプライン使用時のみ必要）、`tqdm`（プログレスバー。無い場合は簡易テキスト進捗に自動フォールバック）

## インストール

```bash
pip install mobi          # 必須
pip install pillow tqdm   # 任意: --img-edit パイプライン / プログレスバー（無ければテキスト進捗にフォールバック）
```
...
### その場で向きを修正（向き異常ページを EXIF で自動回転、CBZ その場修正モード）

```bash
python manga-mobi2cbz.py "D:\ComicsLibrary\Vol.01.cbz" --img-edit rotate
```

`--img-edit` 画像処理パイプラインは mobi→cbz 変換出力前と `--repack` パッキング前にも適用されます。rotate 値なし（auto）は EXIF Orientation で向き異常画像を自動回転します（v3.6.0）。
...
| `--drop [EXPR]` | 統一破棄エントリ（`nargs='?'`、繰り返し指定 = OR 和集合）。値なし = `extra`（未収集の余分な画像をすべて破棄。デフォルトは cbz 末尾に追記）。値を指定すると条件で破棄 — 形式ワード / 条件ワード（`res` / `size` / 向き / モード / ビット深度 / タグ: `double` / `thumbnail` / `animated` / `cover` など）/ `small[=比率]` 小画像。複数条件はカンマ = OR、`+` = AND、`-` 接頭辞 = 除外。例: `--drop gif,extra`、`--drop small=0.4`、`--drop 表紙+overscale`。`off` / `no` / `0` で無効化。実行順: 余分な画像の破棄 → 重複除去 → 条件フィルタ → 小画像の破棄。破棄したファイル名を 1 件ずつ列挙（`--short-summary` 併用時は件数のみ）。旧 `--drop-extra` / `--drop-small` は本オプションに統合（隠しエイリアスとして引き続き利用可） |
...
| `--inspect [MODE][,FILTER]` | 検査モード。`sample` で 1 冊ランダム検査（デフォルト）、`all` で全冊検査。フィルタを付与可能（例: `all,small=0.6`）で、条件に一致した画像の枚数 + ファイル名一覧（サイズ付き）を出力。位置引数が単一ファイルの場合はそのファイルを直接検査し、内部情報のみ読取（CBZ 非生成、一時ディレクトリは終了時に削除）。画像プレビューは先頭 5 枚を表示し、それ以上ある場合は英語の省略記号行 `...`（例: `...（全 N 枚）`）を追記。目次（NCX）/ nav プレビューの省略も英語 `...` に統一。本単位サマリーに「向き異常 N 枚（rotate=auto で回転予定）」行を追加（`--json-out` は `rotate_count` フィールド）、フィルタは方向ワードをサポート（例: `all,方向`）（v3.6.0） |
...
| `--unpack` | 解凍表示: 変換せず解凍のみ。各ソースファイルと同じ名前のサブディレクトリへ出力（既存時は `(2)(3)` と自動採番。`--output-dir DIR` で解凍出力先を指定可、未指定時は各ソースファイルのあるディレクトリが既定（v3.6.0））。mobi は extract で完全な構造を保持、cbz は extractall（zip-slip パストラバーサル対策付き）。`--unpack` 指定時は `.cbz` 入力も収集 |
...
| `--img-edit [EDIT]` | 画像処理パイプライン（繰り返し指定可、値内の '+' で分割してフラット化）：`rotate[=auto |
| `--pages PAGES` | ページ指定処理（繰り返し指定可、値内のカンマ区切りで引用符不要）：指定したページ番号のみ処理。ページ番号は 1 始まり（第 1 ページ = 巻内の 1 枚目）。単一 `5`｜閉区間 `1-3`｜開区間 `7-*`（* = 最終ページ）をサポートし、カンマで混在可（例: `1-3,5,7-*`）。範囲外ページは無視して 1 ページごとに警告、全範囲外 / 空ヒットはエラー終了。`--img-edit`（選択ページのみパイプライン処理）、`--drop`（AND 共存: ページ確定後にフィルタ）、`--unpack` / `--repack`（選択ページのみ）と連携（v3.6.0）。フィルタ語でもページを選択可能：`name=` の値に `*` か `?` を含むと glob 一致（例 `name=*_表紙*`、`name=p00?`）、他の語彙は `--drop` と同じ |
| `--list-images [FILTER]` | 画像一覧の読み取り専用表示: 値なし = 対象電子書籍の全画像を列挙。値を指定すると FILTER 式（形式 / `res` / `size` / 向き / モード / ビット深度 / タグ / `small[=比率]`。カンマ = OR、`+` = AND、`-` 接頭辞 = 除外。例: `jpeg,size>1MB`、`-webp`）。読み取り専用スキャン — 変換なし、CBZ 生成なし、ComicInfo なし、ファイル書き込みなし。CBZ は zip 直接読み取り（ディレクトリ列なし / 変換状態タグなし）。ファイルごとに（No. / ファイル名 / 解像度 / サイズ / モード・深度 / 向き / ディレクトリ / タグ）の一覧 + 全量統計ブロック（フィルタの影響を受けず常に全量）。EXIF Orientation≠1 の画像には `[回転N]` マーク（N=2~8、正常=1 は無印）を付け `[異常]` 集計にも加算。方向ワードによるフィルタ（同義: orient/orientation/direction、日本語=方向/方向異常）を新設: 値なし = 方向異常の全画像を選択（rotate=auto で回転予定）、値あり = EXIF Orientation の正確一致（例: `方向=6`）。`--json-out` の画像レコードに `orientation` フィールドを追加（v3.6.0）。`--json`（ファイルごとに 1 行のスリム JSON）/ `--quiet`（明細抑制、件数のみ）と併用可。`--inspect` とは完全に独立 |
...
一部のパラメータはモードによって効きません。該当しないモードでは黙って無視されます（エラーも警告も出ません）。以下が正となります：

| パラメータ | mobi→cbz 変換 | `--unpack` | `--list-images` | `--repack` | CBZ その場修正 |
| --- | --- | --- | --- | --- | --- |
| `--img-edit` | ✅ 出力前パイプライン | ⏭ 無視 | ⏭ 無視 | ✅ 画像書き込み前にパイプライン | ✅ その場再エンコード |
| `--drop` | ✅ 破棄/フィルタ | ⏭ 無視 | ⏭ 無視（読み取り専用フィルタは FILTER 語を使う） | ⏭ 無視 | ⏭ 無視 |
| `--pages` | ✅ 選択ページのみ | ✅ 選択ページのみ残す | ⏭ 無視 | ✅ 選択ページのみパッキング | ✅ 選択ページのみ |
| `--double-page` | ✅ DoublePage マーク書き込み | ⏭ 無視 | ⏭ 無視 | ✅ ComicInfo 引き継ぎ時は既存マーク維持 | ⏭ 無視 |
| `--rename` | ✅ 出力ファイル名変更 | ⏭ 無視 | ⏭ 無視 | ⏭ 該当なし | ✅ その場リネーム |
| `--setinfo` | ✅ ComicInfo 上書き | ⏭ 無視 | ⏭ 無視 | ✅ ComicInfo 上書き/生成 | ✅ その場 ComicInfo 編集 |
| `--no-comicinfo` | ✅ ComicInfo 生成なし | ⏭ 無視 | ⏭ 無視 | ✅ ComicInfo 引き継ぎなし | ⏭ 無視 |
| `--delete` | ✅ 成功後に元ファイル削除 | ⏭ 無視 | ⏭ 無視 | ⏭ 該当なし | ⏭ 該当なし |

> 注: `--double-page` / `--rename` / `--setinfo` / `--no-comicinfo` / `--delete` の行は実際のコード挙動に基づきます。`--unpack` は既存 `.cbz` / `.epub` 入力（プレーン zip 解凍）も扱い、`--pages` 指定時は解凍後に選択ページの画像だけを自然ソート順の番号で残します。
...
- **`--pages small` が解凍/再パッキング経路で無効** — `_dir_attrs_roll` が `None` を直書きし small 比率が失われていました。`--pages` 式から一様に抽出するよう修正（値なし = 既定比率、値あり = 指定値）、`_cbz_attrs_roll` と同型
- **`--unpack` が mobi 解凍後に mobi7/mobi8 ディレクトリを特定せず番号付け** — ツリー全体でページ番号を振るため、両ディレクトリを跨いで誤削除/誤選択の恐れ。解凍直後に `select_mobi_dir` を実行してからページ処理するよう修正
- **`--repack` で出力ディレクトリが存在しないと失敗** — パッキング前に出力ディレクトリを自動作成
- **極小画像で `--img-edit` の白辺トリミングがクラッシュ** — 幅または高さが 3px 未満で四隅サンプリングが IndexError。中央ピクセルにフォールバック
- **混合入力（mobi 変換 + CBZ 修正）で CBZ 修正失敗が破棄** — 最終終了コードに `cbz_failed` を組み込み、いずれかが失敗すれば非ゼロを返す
- **トップレベルの `mobi` ハード依存チェックが `--help` / `--version` を誤爆** — 遅延ロード（`_require_mobi`）に変更し、実際に mobi を解凍するモードのみライブラリ不足で終了
- **プレビュー統計が `run.error` の文言を再利用** — 独立キー `img_edit.preview_fail` を四言語に追加
  
  #### ドキュメント
  
- **未実装の `denoise` / `whitebalance` パイプライン約束を除去** — 四言語ヘルプと README の固定順序を実際の実装 `rotate→flip→grayscale→format→quality→scale→trim→strip` に統一（denoise/whitebalance はコメント内の予約語のみ）
- **zh-TW `help.output_dir` を他三言語と同期** — `--unpack` の出力先指定を明確化
- **パラメータ × モード有効行列を追加** — `--img-edit` / `--drop` / `--pages` 等が各モード（変換 / `--unpack` / `--list-images` / `--repack` / その場修正）で有効かを明文化（「パラメータとモードの有効行列」参照）
  
  #### 内部
  
- `ebook_to_cbz` の未使用パラメータ `delete_original` を削除

### [3.6.1] - 2026-09-07

#### 修正

- **`--pages` のフィルタ語 `small` が効かない問題** — mobi 変換と CBZ その場修正の経路で `_fill_small_mark` が小画像マークを付与していなかった（`None` を渡していた）。`--pages` 式から small 比率を一元的に抽出するように修正
- **`--delete` と変換タイムアウトの並行でソース削除が誤発火する可能性** — 削除をメインスレッドに集約し、変換成功の確定後にのみ実行。タイムアウトで発生した `mobi.extract` の孤児スレッドはソースを削除しない
- **`--setinfo` / `--img-edit` / `--rename` の純 CBZ 修正で失敗しても終了コードが 0 のまま** — 各モードの失敗ファイル数に応じて非ゼロ終了コードを返す（成功/スキップ = 0）
- **CBZ その場修正のエラー文言が欠落（`run.error` を四言語で補完）** — KeyError を解消
- **プレビュー統計の例外が黙って握り潰されていた** — `_preview_img_edit_count` がエラー表示後に 0 を返すよう修正
  
  #### 内部
  
- 無効な `drop_extra_hit` 変数を削除

### [3.6.0] - 2026-09-05

#### 追加

- **`--pages` ページ指定処理** — 指定ページのみ処理（ページ番号は 1 始まり、第 1 ページ = 巻内の 1 枚目）。繰り返し指定可（和集合）、値内のカンマ区切りで引用符不要：単一 `5`｜閉区間 `1-3`｜開区間 `7-*`（* = 最終ページ）とカンマ混在（`1-3,5,7-*`）をサポート。範囲外ページは無視して 1 ページごとに警告、全範囲外 / 空ヒットはエラー終了。`--img-edit`（選択ページのみパイプライン）、`--drop`（AND 共存）、`--unpack` / `--repack`（選択ページのみ）と連携
- **`--img-edit` 画像処理パイプライン** — 出力画像に回転 / 鏡像反転 / グレースケール化 / 形式変換 / 再エンコード品質制御 / 拡縮 / 白辺トリミング / EXIF 除去を適用。三つの入口：mobi→cbz 変換出力前 / `--repack` パッキング前 / 既存 `.cbz` のその場修正（CBZ 方向修正モード）。`rotate[=auto|90|180|270]` は前述のとおり向きを処理；`flip=x|y|both` は鏡像反転；`grayscale` はグレースケール化（常に flip の直後に挿入、出力は RGB 三チャンネル等値）；`format=jpeg|png|webp`（別名 jpg；目標形式、再エンコード後に cbz 内エントリの拡張子を同期変更；透明→JPEG はデフォルトで白を補填、`format=jpeg,black` や `#RRGGBB` で色変更可；JPEG/WebP は quality で保存、PNG は可逆）；`quality=1-100` は再エンコード品質を上書き；`scale=NNN%` はパーセント相対拡縮（ページ間の比率を維持、例 `200%`=2 倍、1~1000）または `scale=NNNw` は目標幅ピクセルに統一（高さは元の比率、例 `1200w`、1~100000、二つの構文は二択）；`trim[=許容差]` は辺縁の均一背景色を検出して白辺を自動トリミング；`strip` は画像の全 EXIF メタデータを除去（常にパイプライン末尾）；固定順序 `rotate→flip→grayscale→format→quality→scale→trim→strip` は記述順に関係なし
- **`name=` ファイル名 glob フィルタ** — `name=` の値に `*` / `?` を含む場合は glob 一致でファイル名をマッチ（大文字小文字を無視、特別扱いは `*` と `?` のみ、`[]` はリテラル）、含まない場合は従来の部分一致を維持。`--list-images` / `--drop` / `--pages` の三入口で有効、例 `name=*_表紙*`、`name=p00?`
- **`--drop` の繰り返し指定を追加（OR 和集合）** — フラグを繰り返すと累積（`--drop a --drop b` = 両条件の OR）。旧 `--drop-extra` は隠しエイリアスとして維持
- **`--unpack` が `--output-dir` に対応** — 出力ディレクトリを指定可能。未指定時はソースファイルのあるディレクトリに出力
- **`--list-images` の `[回転N]` マーク・方向ワード・`orientation` フィールド** — EXIF Orientation≠1 の画像に `[回転N]` マークを付け `[異常]` 集計に加算。方向ワード：値なし = 方向異常の全画像を選択（rotate=auto で回転予定）、値あり = EXIF Orientation の正確一致（例: `方向=6`）。`--json-out` は `orientation` フィールド
- **`--inspect` の本単位回転数サマリー** — 「向き異常 N 枚（rotate=auto で回転予定）」行と `info.rotate_count` フィールド（`--json-out` 出力）を追加。方向ワード対応（例: `all,方向`）
  
  #### 修正
  
- **ビッグエンディアン TIFF EXIF Orientation の解析を修正** — SHORT 値を 4 バイトとしてマスク判定していた問題を修正し、バイトオーダーに応じて 2 バイトを解読
