...
- **ディレクトリ整合フォールバック** — ディレクトリ内の画像数が収集数と一致しない場合、余分な画像はデフォルトで自然順に cbz 末尾へ追記します。`--drop-extra` で破棄に変更でき、破棄した余分な画像のファイル名を 1 件ずつ列挙します（`--short-summary` 併用時は件数のみ）
...
- **画像一覧の読み取り専用表示** — `--list-images [FILTER]` で対象電子書籍の全画像を（No. / ファイル名 / 解像度 / サイズ / モード・深度 / 向き / ディレクトリ / タグ）で列挙し、全量統計ブロック（形式 / モード・深度 / 寸法分布 / 見開き横幅 / 動図 GIF / 小画像 / 異常明細）を付加。変換せず、CBZ 書き込みも ComicInfo 生成もしません。FILTER は任意で条件式をサポート（形式 / `res` / `size` / 向き / モード / ビット深度 / タグ。カンマ = OR、`+` = AND、`-` 接頭辞 = 除外）
...
- **小画像の破棄** — `--drop-small` で幅・高さとも 中央値×比率（デフォルト 0.5）未満の異常な小画像（表紙サムネイル等）を除外します。破棄後は PageCount を実画像数で再計算。値なし / `auto` = 0.5、0〜1 の数値で比率調整、`off` / `no` / `0` で無効
...
| `--drop-extra FILTER`   | 汎用破棄フィルタ（`nargs='?'`）。値なし = `extra`（未収集の余分な画像をすべて破棄。旧動作と同等。デフォルトは cbz 末尾に追記）。値を指定すると条件で破棄 — 形式 / `res` / `size` / 向き / モード / ビット深度 / タグ（`double` / `thumbnail` / `animated` / `small` / `cover`。`cover` は OPF guide の表紙 + ファイル名キーワードで判定）（`--list-images` と同じ式エンジン、カンマ = OR、`+` = AND。例: `--drop-extra gif`、`--drop-extra gif,extra`、`--drop-extra cover`）。`off` / `no` / `0` で無効化。実行順: 余分な画像の破棄 → 重複除去 → 条件フィルタ → 小画像の破棄。`--drop-small` と併用可能。破棄したファイル名を 1 件ずつ列挙（`--short-summary` 併用時は件数のみ）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `--inspect`             | 検査モード。`sample` で 1 冊ランダム検査（デフォルト）、`all` で全冊検査。位置引数が単一ファイルの場合はそのファイルを直接検査し、内部情報のみ読取（CBZ 非生成、一時ディレクトリは終了時に削除）。画像プレビューは先頭 5 枚を表示し、それ以上ある場合は英語の省略記号行 `...`（例: `...（全 N 枚）`）を追記。目次（NCX）/ nav プレビューの省略も英語 `...` に統一                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- **新 `--list-images [FILTER]` 読み取り専用画像一覧** — 対象電子書籍の全画像を（No. / ファイル名 / 解像度 / サイズ / モード・深度 / 向き / ディレクトリ / タグ）で列挙し、全量統計ブロック（形式 / モード・深度 / 寸法分布 / 見開き横幅 / 動図 GIF / 小画像 / 異常画像の明細）を付加。変換なし、CBZ 生成なし、ComicInfo なし、ファイル書き込みなし。`--inspect` とは完全に独立。値なし = 全て列挙、値あり = FILTER 式（形式 / `res` / `size` / 向き / モード / ビット深度 / タグ。カンマ = OR、`+` = AND、`-` 接頭辞 = 除外。例: `jpeg,size>1MB`、`-webp`）。フィルタは一覧行のみに影響し統計は常に全量。CBZ は zip 直接読み取り（ディレクトリ列なし / 変換状態タグなし）。`--json`（ファイルごとに 1 行のスリム JSON）と `--quiet`（明細抑制、件数のみ）に対応
