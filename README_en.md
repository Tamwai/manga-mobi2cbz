...
- **Directory alignment fallback** — when the image count in the directory differs from the collected count, extra images are appended to the end of the cbz in natural order by default; `--drop` switches to dropping them instead, and the extra image file names are listed one by one (only counts under `--short-summary`)
...
### In-place orientation fix (auto-rotate orientation-anomalous pages by EXIF; CBZ in-place fix mode)

```bash
python manga-mobi2cbz.py "D:\ComicsLibrary\Vol.01.cbz" --img-edit rotate
```

The `--img-edit` image processing pipeline also applies before mobi→cbz conversion output and before `--repack` packing; `rotate` with no value (auto) auto-rotates orientation-anomalous images by EXIF Orientation (v3.6.0).
...
| `--drop [EXPR]` | Unified drop entry (`nargs='?'`, repeatable = OR union): no value = `extra`, dropping all uncollected extra images (appended to the end of the cbz by default); with a value, drops by conditions — format words / condition words (`res` / `size` / orientation / mode / bit depth / tag: `double` / `thumbnail` / `animated` / `cover` etc.) / `small[=ratio]` small images; multi-condition comma = OR, `+` = AND, `-` prefix excludes, e.g. `--drop gif,extra`, `--drop small=0.4`, `--drop cover+overscale`; `off` / `no` / `0` disables; execution order: drop extra images → dedupe → filter → drop small images; dropped file names are listed, only counts under `--short-summary`; the old `--drop-extra` / `--drop-small` are merged into this option (hidden aliases, still usable) |
...
| `--inspect [MODE][,FILTER]` | Inspect mode: `sample` randomly inspects 1 ebook (default), `all` inspects every file; a filter may be attached (e.g. `all,small=0.6`) and matched images print a count + filename list (with dimensions); a single-file positional argument inspects that file directly; unpack only to read internal info (metadata/structure/images/resolution/dual DRM judgment/NCX TOC), no CBZ produced, temp directory cleaned up automatically; the image preview lists the first 5 files and, when there are more, appends an English ellipsis line `...` (e.g. `... (188 images)`); TOC (NCX) / nav preview truncation is also unified to English `...`; the book-level summary adds a “N rotated image(s) (rotate=auto will rotate)” line with a `rotate_count` field carried out via `--json-out`, and orientation filter words are supported (e.g. `all,orientation`) (v3.6.0) |
...
| `--unpack` | Unpack to view: extract only, no conversion; outputs to a same-name subdirectory next to each source file (auto-numbered `(2)(3)` if it already exists; `--output-dir DIR` sets the unpack output directory, defaulting to each source file's directory when omitted, v3.6.0); mobi uses extract preserving the full structure, cbz uses extractall (with zip-slip path-traversal protection); `.cbz` inputs are also collected |
...
| `--drop small[=ratio]` | Drop small images: exclude images whose area is clearly small during conversion (cover thumbnails / copyright pages, etc.) — an image is dropped when width × height < area median × ratio; `small` omitted or `auto` uses the default ratio 0.5, a `0~1` number (e.g. `--drop small=0.4`) adjusts it, `off` / `no` / `0` disables (disabled by default, existing behavior unchanged); ComicInfo `PageCount` is recomputed from the remaining images; summary / `--log` / `--json` report a "dropped small" count (`--json` emits a `dropped_small` field) and list the dropped file names one by one (only counts under `--short-summary`); `--inspect` preview shows "N image(s) will be dropped when --drop small is enabled"; wide double-page banners (width not small) are never mis-dropped; conversion mode only; the old `--drop-small` ≡ `--drop small` (hidden alias, still usable) |
| `--img-edit [EDIT]` | Image processing pipeline (repeatable; split ops with '+' inside a value): `rotate[=auto |
| `--pages PAGES` | Page selection (repeatable; comma-separated inside one value, no quoting needed): process only the selected pages, page numbers start at 1 (page 1 = the 1st image in the volume). Supports single `5` |
| `--list-images [FILTER]` | Read-only image listing: no value = list all images of the target ebook; with a value = FILTER expression (format / `res` / `size` / orientation / mode / bit depth / tag / `small[=ratio]`; comma = OR, `+` = AND, `-` prefix excludes, e.g. `jpeg,size>1MB`, `-webp`); read-only scan — no conversion, no CBZ, no ComicInfo, no files written; CBZ read directly from zip (no TOC column / no conversion-state tags); per-file block (No. / filename / resolution / size / mode·depth / orientation / TOC / tag) + full statistics block (always full-set, unaffected by the filter); images with EXIF Orientation≠1 carry a `[rotateN]` mark (N=2–8, normal=1 not marked) also counted into the `[anomaly]` summary, and orientation filter words are available (`orient` / `orientation` / `方向` / `方向異常`): no value = select all orientation-anomalous images (rotate=auto will rotate), a value = exact EXIF Orientation match (e.g. `orientation=6`); `--json-out` image records add an `orientation` field (v3.6.0); works with `--json` (one slim JSON per file) and `--quiet` (detail suppressed, count only); independent from `--inspect` |
...
- **`--list-images` `[rotateN]` mark, orientation filter words, and `orientation` field** — images with EXIF Orientation≠1 show a `[rotateN]` mark and are counted in the `[anomaly]` summary; orientation filter words: no value = select all orientation-anomalous images (rotate=auto will rotate), a value = exact EXIF Orientation match (e.g. `orientation=6`); `--json-out` adds an `orientation` field
- **`--inspect` book-level rotation total summary** — adds a "N rotated image(s) (rotate=auto will rotate)" line and an `info.rotate_count` field carried out via `--json-out`; orientation filter words supported (e.g. `all,orientation`)
  
  #### Fixes
  
- **Big-endian TIFF EXIF Orientation parsing** — fixed the bug that misread SHORT values as 4 bytes with a mask; now decodes 2 bytes according to byte order
...
- **`--drop [EXPR]` unified drop entry** — no value drops extra images outside the collection (same semantics as the old `--drop-extra` with no value); format words / condition words / `small[=ratio]` all accepted; multi-condition comma = OR, plus = AND, `-` prefix excludes, `off` disables; the old `--drop-extra` / `--drop-small` are demoted to hidden aliases and merged in
- **`--inspect [MODE][,FILTER]`** — on top of sample/all, a filter may be attached (e.g. `all,small=0.6`); matched images print count + filename list (with dimensions); `--json` gains a matching `filter_hits` structure
- **`small` standalone parameterized condition word** — no value / auto = default ratio 0.5, a 0~1 number sets the ratio; the four-language aliases (异常小图 / 異常小圖 / 異常小画像 / 極小画像) all accept a ratio
  
  #### Changes
  
- The three pipelines (conversion drop / inspect preview / list) are all driven by the `--drop` expression; the small-image area criterion has a single source
- `--drop-extra` / `--drop-small` are hidden from `--help` (still usable for compatibility)
  
  #### Maintenance
  
- Regression tests increased to 46 cases

### [3.2.0] - 2026-08-24

#### New features

- **[abnormal] always-on inline marker** — shown in the first column as a rollup of any anomaly in that row, visible at a glance
- **[inferred] dedicated inference marker, replacing "疑似"** — inference-based annotations (rotated double-pages / thumbnails / cover fallback) drop the "疑似" tag: entries inferred only from size / filename and not explicitly declared by the OPF are marked `[inferred]` (OPF guide covers / animated images / extras are not marked); new four-language filter aliases `inferred` / `推断` / `推斷` / `推測` / `推定`
  
  #### Changes
  
- **Mode column unified to canonical English names** — `index` / `gray` / `rgb` / `graya` / `rgba`; the four-language localization keys are removed to reduce maintenance
- **Small-image detection switched to an area-based criterion** — width × height < area median × ratio, unified with the `--drop-small` ratio — what is marked gets dropped; the `--list-images` / `--inspect` preview / conversion-drop pipelines now share one criterion
- **`[small]` is no longer marked when `--drop-small` is off** — consistent with "what is marked gets dropped"
  
  #### Maintenance
  
- Regression tests synchronized (34 cases, all passing)

### [3.1.1] - 2026-08-24

#### Fixes

- `--unpack` help text still carried the old "auto numbered-avoidance" wording, contradicting the unified `source_ext` naming since v3.1.0; corrected in all four languages
  
  #### Maintenance
  
- `--repack` help now notes that `--rename` does not apply to this mode; `--list-images` help now explains the [append] / [drop] / [filter] disposition markers
- Regression tests expanded to 34 cases (three new groups: repack restore + ComicInfo overlay, `_safe_zip_extract` `C:foo` escape, `name` atom on WindowsPath)
