...
- **自定义输出目录** — `--output-dir DIR` 将 CBZ 输出到指定目录（自动创建），不再强制与源 mobi 同目录
- **预处理过滤** — 0 字节、文件头损坏（偏移 60 处无 `BOOKMOBI` 魔数）的 mobi 在预处理阶段直接跳过，日志输出跳过文件完整路径与原因
- **大小下限过滤** — `--min-size BYTES` 过滤小于指定字节数的 mobi（不带数字默认 1000，`0` 关闭，不传则关闭大小过滤），兜住头部恰好完整但内容被截断的边缘损坏样本
...
| `--min-size BYTES` | 过滤小于指定字节的 mobi；不带数字默认 1000，`0` 关闭，不传则关闭大小过滤 |
...
- 默认转换后的 `.cbz` 文件与原 `.mobi` 文件在同一目录；指定 `--output-dir` 时输出到该目录（自动创建）
- 已存在的 `.cbz` 默认自动跳过，不会覆盖；加 `--overwrite` 可强制重新生成
- 0 字节 / 文件头损坏的 mobi 在预处理阶段直接跳过，日志输出完整路径与原因
...
- `--output-dir DIR`：CBZ 输出到指定目录（自动创建），不再强制与源 mobi 同目录；`--overwrite` 存在性判断同样基于输出目录
